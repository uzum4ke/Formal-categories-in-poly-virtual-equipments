...
Counitality gives the factorization triangle
\[
\begin{tikzcd}[column sep=3em]
...
A cell $s\in P(\Omega;T)$ is \emph{contextually universal} if for
...
\end{lemma}
\begin{proof}
Using equivariance, the two balance maps of
...
\end{tikzcd}
\]
To verify its full cartesian property,
...
We call it \emph{bicomodule-carried} when $H_M$ consists of one actual
bicomodule.
\end{definition}
...
distributor. The two equations in \eqref{eq:distcoeqactions} are its exterior
...
of the particular fork~\eqref{eq:distcoequalizer}.
\end{proof}
...
\end{tikzcd}
\]
Tensor product of abelian groups preserves coequalizers in either variable, and finite
...
\Cref{thm:distcoequalizer} equips the quotient in
...
\end{corollary}
\begin{proof}
By \cref{strong:lem:triangular-data}, $F$ is equivariant. It preserves
...
factorization does not exist by \cref{strong:thm:strong}.
\end{proof}
...
\cref{strong:prop:restricted-action}. \Cref{strong:lem:unstable-cotensor}
