...
\newcommand{\harr}{\mathrel{\rightsquigarrow}}
...
\begin{document}
\begin{abstract}
Aguiar's non-Cartesian internal categories use cotensor products of
bicomodules to express composition. His regularity condition supplies
these composites but excludes monoidal categories such as abelian groups.
We define formal internal categories in every monoidal category using
actual bicomodule carriers and a poly-virtual calculus whose two boundaries
are finite paths. Multiplication and units satisfy the monad equations by
tree substitution. The construction forms a forest envelope of balanced
bicomodule maps and makes intrinsic globular contextually universal comparisons reversible.
We prove conservativity, including equivariant maps over changing
comonoid bases, and recover cotensor categories and functors whenever the
relevant cones are stable under tensoring.

We then construct a poly-virtual equipment whose objects are these formal
categories, whose vertical arrows are formal functors, and whose horizontal
arrows are formal distributors. Distributor carriers are finite bicomodule
paths; those with one bicomodule as carrier recover the original bimodule
action data exactly. Balanced distributor multicells form a virtual
equipment: restrictions and their actions are constructed by cartesian
factorization. A second conservative completion supplies many-sided cells,
companions, conjoints, and coherent boundary transport. Contextually
preserved coequalizers represent distributor composition. In the finite
group-like abelian-group sector the resulting calculus recovers enriched
categories, functors, and distributors. The general completion also
...
\end{equation}
satisfying the usual monad equations. The construction makes each existing intrinsic globular
...
\emph{contextually universal} means that a comparison represents its
...
The term \emph{poly-virtual equipment} has the precise meaning given in
...
\Cref{sec:bicomod} identifies the bicomodule cells available before
composition. \Cref{sec:typed,sec:monads} define the two-sided calculus
and its category structures. The forest construction and its conservative
localization occupy \cref{sec:forest,sec:localization};
\cref{sec:global,sec:completion} establish vertical transport and the
general completion theorem. We then construct $\Q(\V)$ in
\cref{sec:formalinternal} and prove cotensor recovery, including changing
bases and actions, in \cref{sec:recovery,sec:changing}.
\Cref{sec:distributors} constructs the virtual equipment of formal
distributors, and \cref{sec:distributorcompletion} gives its conservative
poly-virtual completion. Carrier representation and distributor composition
are treated in \cref{sec:distributorrecovery}. \Cref{sec:examples} tests
both constructions on familiar bases and on $\Ab$. The generalized-
multicategory applications and comparison with other completions follow in
...
from its carrier and its underlying square. The word \emph{atomic} is
...
The enriched precedent separates a calculus of operations from the
objects that represent its horizontal composites. We first make this
separation explicit, then identify the corresponding bicomodule data.
The outcome is a virtual equipment in every monoidal category, together
with the variance issue that the two-sided construction must resolve.
...
$R:\mathcal B\harr\mathcal C$, the usual represented composite has
components
\[
...
\begin{equation}\label{eq:bicomodcommute}
 (1_C\otimes\rho_M)\lambda_M=(\lambda_M\otimes1_D)\rho_M.
\end{equation}
A map $F:M\to N$ over comonoid maps $f:C\to C'$, $g:D\to D'$
is equivariant if
\begin{equation}\label{eq:equivariant}
 \lambda_NF=(f\otimes F)\lambda_M,\qquad
 \rho_NF=(F\otimes g)\rho_M.
\end{equation}
\end{definition}
...
maps. For $n>0$, a balanced cell is a map
$\phi:N\to M_1\otimes\cdots\otimes M_n$ such that
\begin{align}
 (\lambda_{M_1}\otimes1)\phi&=(a\otimes\phi)\lambda_N,
 & (1\otimes\rho_{M_n})\phi&=(\phi\otimes b)\rho_N,
 \label{eq:exterior}\\
 (1\otimes\rho_{M_i}\otimes1)\phi
 &=(1\otimes\lambda_{M_{i+1}}\otimes1)\phi
 &&(1\leq i<n).\label{eq:balance}
\end{align}
The two sides of each interior equation land in the tensor with $C_i$
inserted between $M_i$ and $M_{i+1}$.
For $n=0$ the upper path is $\emp C$, $a:D\to C$, $b:E\to C$,
and a cell is a map $\phi:N\to I$ satisfying
\begin{equation}\label{eq:nullbalance}
 (a\otimes\phi)\lambda_N=(\phi\otimes b)\rho_N:N\to C.
\end{equation}
The unitors identify the codomains with $C$.
\end{definition}
...
\end{proposition}
\begin{proof}
Regard $\V^\op$ as the one-object virtual double category whose cells
...
Explicitly, substitution composes $\phi$ with the tensor product of the
...
remaining equality is \cref{eq:balance} for $\phi$. An empty block
is removed using its map to $I$; equation \cref{eq:nullbalance}
...
blocks proves the statement also when every block is empty. The exterior
...
Restrictions are explicit. To restrict $N:D\harr E$ along vertical
...
\end{lemma}
\begin{proof}
First consider a globular nullary cell $\phi:N\to I$. By
\cref{eq:nullbalance},
\[
 q=(1_C\otimes\phi)\lambda_N
   =(\phi\otimes1_C)\rho_N:N\to C.
\]
Coassociativity of $\lambda_N$ gives
...
Counitality gives $\eps_Cq=\phi$. Conversely a bicomodule map
$q:N\to C$ satisfies both displayed expressions with
$\phi=\eps_Cq$, proving uniqueness.

For a contextual instance, take a balanced map
...
equation supplies the missing endpoint calculation. Coassociativity
gives balance at the two new interfaces, and commuting coactions give
the unchanged exterior equations. Removing the inserted factor by
...
For nonidentity exterior maps, corestrict the two exterior coactions
...
Thus units and restrictions already exist without cotensors. After
reversing squares, the same data give forward equivariant maps and
primitive cells $N\cell(M_1,\ldots,M_n)$. The remaining task is to
place these cells in a two-sided calculus that also admits multiplication
$(A,A)\cell A$ and recognizes any cotensors that are represented.

\section{Two-sided cells and tree pastings}\label{sec:typed}

Balanced bicomodule cells have one input after reversal, whereas a
category multiplication has one output. Both can be treated in a
calculus with paths on the two boundaries. We distinguish tree pastings,
which suffice for category equations, from composition along whole paths,
which will be used to construct the examples. Empty boundaries are
included so that units have their usual nullary form.
...
\begin{tikzcd}[column sep=huge,row sep=large]
 X \arrow[r,rightsquigarrow,"\Gamma"] \arrow[d,"f"'] &
 Y \arrow[d,"g"]\\
 X' \arrow[r,rightsquigarrow,"\Delta"'] &Y'.
 \arrow[Rightarrow,from=1-1,to=2-2,"\alpha" description,shorten <=9pt,shorten >=9pt]
\end{tikzcd}
\]
...
The \emph{wire graph} of a scheme is the finite multigraph whose vertices
...
\emph{tree scheme} is a rectangular scheme whose wire graph is nonempty,
...
a vertical arrow. Call these \emph{elementary structural schemes}.
...
\end{lemma}
\begin{proof}
Rectangular substitution preserves every boundary label and replaces one
...
graph. Substitution of finite labelled rectangles is associative before
...
rectangle. A single identity wire removes a vertex having one input and
...
The last assertion is the resulting associativity identity.
\end{proof}

\begin{definition}[Poly-virtual and merge calculi]\label{def:poly}
A \emph{poly-virtual double calculus} on a boundary signature assigns a
...
A \emph{merge calculus} has evaluation of all rectangular schemes, with
the same equations, including all structural schemes on arbitrary paths.
Its \emph{poly-virtual reduct} $\Up\Bcal$ keeps the
...
called \emph{atomic} when a calculus is presented using their paths.
\end{definition}
...
\end{proposition}
\begin{proof}
An outer single-output rectangle and a compatible family of single-output
rectangles above its input ports form a rooted tree. Shared lateral maps
are precisely the compatibility data of virtual substitution. Replacing
...
causes no change in this argument. Reversing the upper and lower boundaries
proves the single-input assertion. Vertical maps must be matched for the
whole family; they need not allow the family to be pasted one member at a
time with other members held fixed.
\end{proof}
...
The single-output and single-input fragments thus supply the two
substitution laws needed for the motivating examples. The next section
defines category structure using only the retained tree operations.

\section{Formal categories and their homomorphisms}\label{sec:monads}

The two-sided calculus permits a category structure to be specified before
the path $(A,A)$ has a horizontal representative. Its multiplication,
unit, and homomorphism equations use the single-output substitution
fragment. We show that their entire category of models survives passage
from a merge calculus to its poly-virtual reduct.

\begin{definition}\label{def:formalcat}
A \emph{formal category} in $\Pcal$ is a quadruple $(X,A,\mu,\eta)$,
...
\begin{equation}\label{eq:monadlaws}
 \mu(\mu,1_A)=\mu(1_A,\mu),\qquad
 \mu(\eta,1_A)=1_A=\mu(1_A,\eta).
\end{equation}
...
\begin{equation}\label{eq:functorlaws}
 F\mu_A=\mu_B(F,F),\qquad F\eta_A=\eta_B[f].
\end{equation}
Here $\eta_B[f]$ is the nullary cell obtained by precomposing its upper
vertex by $f$, that is, by pasting the structural empty square on $f$.
\end{definition}

In a merge presentation the equations can be displayed as
\[
\begin{tikzcd}[column sep=large]
 (A,A,A) \arrow[r,"\mu\wh1"] \arrow[d,"1\wh\mu"'] &
 (A,A) \arrow[d,"\mu"]\\
 (A,A) \arrow[r,"\mu"'] & A
\end{tikzcd}
\qquad
\begin{tikzcd}[column sep=large]
 A \arrow[r,"\eta\wh1"] \arrow[dr,equal] &
 (A,A) \arrow[d,"\mu"] &
 A \arrow[l,"1\wh\eta"'] \arrow[dl,equal]\\
 & A &
\end{tikzcd}
\]
Each complete composite in these diagrams is a tree evaluation and
therefore has meaning in the reduct. The individual whiskerings, such
as $\mu\wh1_A$, are written in the merge presentation; adjoining
an unused identity wire is not itself a designated tree operation.

\begin{proposition}\label{prop:cat}
Formal categories and their homomorphisms form a category $\FCat(\Pcal)$.
\end{proposition}
\begin{proof}
Compose $(f,F)$ and $(g,G)$ by $(gf,GF)$, using unary tree pasting.
Associativity and identities follow from the corresponding tree equations.
For multiplication, the homomorphism equations for $F$ and $G$,
...
\begin{align*}
 ...
 &=(G\mu_B)(F,F)=\mu_C(G,G)(F,F)
   =\mu_C(GF,GF).
\end{align*}
For units, the structural empty squares compose with their vertical
arrows, so
$(GF)\eta_A=G\eta_B[f]=\eta_C[g][f]=\eta_C[gf]$.
Thus the composite satisfies both homomorphism equations. The same
equations hold for identity cells by the substitution identity laws.
\end{proof}
...
\end{theorem}
\begin{proof}
All cells specifying an object or morphism occur in the retained cell
collections. Every operation in \cref{eq:monadlaws,eq:functorlaws} is a
tree or structural operation. The reduct retains their values and the
equalities between them. Finally, identity and composite homomorphisms use
only structural and unary tree operations. Consequently the object sets,
hom-sets, identities, and compositions agree.
\end{proof}

Consequently, merge operations can be used to build a realization
without becoming additional operations in the definition of formal
category. We next construct such a realization from the original
single-output calculus and prove that it preserves that calculus.

\section{The typed forest envelope}\label{sec:forest}

To construct two-sided cells, first place original multicells side by side
as ordered forests. This step uses no represented composites. We then
identify the original comparisons whose universal property allows a path
to be replaced by one arrow in every surrounding context. In the
bicomodule application, this test will recognize stable cotensor cones.
...
\end{lemma}
\begin{proof}
Associativity of substitution proves vertical associativity; concatenation
proves horizontal associativity. Substitution on disjoint consecutive blocks
commutes, which is interchange. The forest of unary identities and the
empty forest give the identity laws. With one output the subdivision has
only one block, proving \cref{eq:forestsingle}. A forest must be sent to
the horizontal composite of the images of its roots, so an extension is
unique, and the same substitution identities show that it is a functor.
\end{proof}

\begin{definition}[Intrinsic comparison class]\label{def:universal}
A cell $s\in P(\Omega;T)$ is \emph{contextually universal} if for all
compatible paths $L,R$ and every atomic output $B$, substitution is a
bijection
\begin{equation}\label{eq:universal}
 s^*:P(L,T,R;B)\longrightarrow P(L,\Omega,R;B),
 \qquad b\longmapsto b(1_L,s,1_R).
\end{equation}
Let $S(P)$ be the class of all such cells, including those with empty
source. We say that $\Omega$ is \emph{represented by $T$} when such an
$s$ exists.
\end{definition}
...
\end{lemma}
\begin{proof}
The first assertion is the identity law. For the second, the substitution
...
maps are bijections. For the last assertion, enlarge $L,R$ in
...
\end{lemma}
\begin{proof}
For a target $B$ and exterior arrows $f,g$, its cartesian restriction gives
\[
 \D_{f,g}(\Xi;B)\cong P(\Xi;B[f,g]).
\]
This bijection commutes with substitution in $\Xi$. Apply it to both
sides of the desired factorization map and use
\cref{eq:universal} with output $B[f,g]$. The converse takes identity
vertical boundaries.
\end{proof}

The forest envelope supplies several-output boundaries, and
$S(P)$ records exactly the comparisons licensed by the original
contextual universal properties. These are the comparisons made
reversible in the next section. Using the whole typed calculus ensures
that the same test also covers paths through different bases.

\section{Conservative localization}\label{sec:localization}

A represented path must be usable on either boundary if its representative
is to support the later multiplication comparison. We therefore adjoin
inverses to the contextual comparisons. The issue is whether this creates
new cells with an original single output, or new equalities between such
cells. Substitution presheaves prove faithfulness, and backward evaluation
proves that every cell of this profile is already an original multicell.
...
\end{lemma}
\begin{proof}
The action of a forest is simultaneous substitution into its consecutive
blocks. Identity and composition follow from the virtual identities.
The assertion about comparisons is precisely \cref{eq:universal}.
Thus the action of a formal inverse is the inverse bijection, and this
defines the extension. If original cells $a,b$ have equal images, apply
...
\end{lemma}
\begin{proof}
Write $z$ as a finite vertical sequence of forests and inverses of
...
identity forests. Start at the final boundary $A$ with $1_A$ and traverse
the sequence backwards. At an ordinary forest $a:\Xi\cell\Psi$,
...
Then in the localization $bt^{-1}=c$. In either case the original cell
now obtained equals the localized suffix that it replaces. Induction over
the finite sequence proves the assertion. These two backward operations
are exactly the actions defining $\overline\Phi_A$, which also proves
independence from the sequence used.
\end{proof}
...
\end{theorem}
\begin{proof}
Injectivity is \cref{lem:presheaf} and surjectivity is
\cref{lem:backward}. The canonical map respects substitution by its
definition. Reversing the displayed bijections proves the dual assertion.
\end{proof}
...
\end{theorem}
\begin{proof}
The reverse implication is the construction. Conversely an isomorphism
$\Gamma\to T$ is the image of a unique original cell $s$ by
\cref{thm:conservative}. Whiskering it by any $L,R$ is an isomorphism.
...
the resulting bijection is \cref{eq:universal}. The same argument applies
to any specified $s$ whose image is invertible.
\end{proof}
...
\end{theorem}
\begin{proof}
Extend first to forests by \cref{lem:forest}. Necessity follows from
\cref{eq:inverserel}. For sufficiency send $s^{-1}$ to the inverse of
the interpreted $s$; all defining equations hold. An inverse, when it
exists, is unique, so all values of the extension are forced.
\end{proof}
...
The usable conclusion is \cref{eq:conservative}: original
single-output cells and their substitutions are unchanged. At the same
time, represented paths can now be used reversibly, with their occurrence
detected by \cref{thm:detection}. The remaining construction extends
these globular results to arbitrary vertical boundaries.

\section{Changing vertical boundaries}\label{sec:global}

Globular conservativity compares cells whose exterior maps are identities.
Internal functors, however, may change the comonoid base. We recover all
vertical boundaries using companion and conjoint adjunctions, then prove
that the resulting square calculus preserves the original changing-base
cells. The construction uses the mixed-base paths already retained in
the forest envelope.
...
their unique binding-compatible comparisons. This is the usual binding or
...
\begin{equation}\label{eq:triangles}
 (1_{f_*}\wh e_f)(j_f\wh1_{f_*})=1_{f_*},\qquad
 (e_f\wh1_{f^*})(1_{f^*}\wh j_f)=1_{f^*}.
\end{equation}
...
\end{lemma}
\begin{proof}
We recall the binding argument, to specify which universal properties are
...
on $f$. The horizontal binding identity is proved by postcomposing with
...
Paste the cartesian companion and conjoint cells on the two ends of
$H$, and remove the displayed copies of $U_{X'}$ and $U_{Y'}$ by their
unit universal properties. Factoring through $H[f,g]$ gives
\cref{eq:restrictioncomparison}. To factor a cell having $f_*Hg^*$
...
For the first assertion, the same argument with $H=U_{X'}$ makes
...
$t_f$ with the inverse representing comparison. The two cartesian
binding cells, followed by unit composition, give
$f^*f_*\cell U_{X'}$; compose with the inverse of
$\emp{X'}\cell U_{X'}$ to obtain $e_f$.
The two composites in \cref{eq:triangles}, on expanding these
...
For composable $f:X\to Y$ and $u:Y\to Z$, paste their companion binding
...
Two comparisons between binding pairs which respect the binding cells are
...
$l$'s. Identity comparisons are proved similarly. Finally the constructions
...
\begin{equation}\label{eq:hcomp}
 \widehat{a\wh b}=
 (1_{f_*\Delta}\wh e_g\wh1_{\Delta'h^*})
 (\widehat a\wh\widehat b).
\end{equation}
If $b$ below $a$ has sides $(u,v)$, define vertical pasting by
\begin{equation}\label{eq:vcomp}
 \widehat{ba}=
 (k_{f,u}\wh1_{\Xi}\wh l_{g,v})
 (1_{f_*}\wh\widehat b\wh1_{g^*})\widehat a,
\end{equation}
...
For example, \cref{eq:vcomp} is the typed sequence
\[
 \Gamma\longrightarrow f_*\Delta g^*
 \longrightarrow f_*u_*\Xi v^*g^*
 \longrightarrow(uf)_*\Xi(vg)^*.
\]
...
\end{theorem}
\begin{proof}
For three horizontally adjacent squares, either association tensors their
representatives and contracts the two disjoint strings $g^*g_*$ and
$h^*h_*$. The contractions commute by interchange in $L(P)$.
Horizontal composition with $j_f$ or $j_g$ is an identity by the two
triangle identities \cref{eq:triangles}.

For three vertically adjacent squares, either association first substitutes
...
these results equal. Their identity comparisons are normalized, proving
the vertical identity laws. Equation \cref{eq:unitcomp} states exactly
that the horizontal identity on $uf$ is the vertical paste of those on
$f$ and $u$.

It remains to check interchange. At an interior vertical side, write its
...
structural subdivisions and hence evaluation of all rectangular schemes.
\end{proof}
...
formulas \cref{eq:hcomp,eq:vcomp}, and taking the class in
...
\end{proposition}
\begin{proof}
For two represented units, universal factorization gives a unique
isomorphism carrying one nullary universal cell to the other. For two
...
$\lambda_{\mathfrak f,\mathfrak f'}$. A composite of two
...
Binding-compatible isomorphisms preserve $j$ and $e$: after expanding
...
pasted binding pairs. Therefore changing any representative or seam in a
...
Evaluate the two sides of any associativity, identity, or interchange law
...
the quotient is a strict double category and a merge calculus. If a
...
the one already proved for the binding-compatible comparisons.
\end{proof}
...
\end{theorem}
\begin{proof}
For the fixed profile in question, choose binding presentations of the two
exterior arrows. By \cref{prop:choices}, every class in
$\E(\D)_{f,g}(\Gamma;H)$ has a unique representative in the corresponding
summand of \cref{eq:choicefreecells}. The desired bijection is therefore
the composite
\begin{align*}
 \D_{f,g}(\Gamma;H)
 &\cong P(\Gamma;H[f,g])\\
 &\cong L(P)(\Gamma;H[f,g])\\
 &\cong L(P)(\Gamma;f_*Hg^*)\\
 &\cong \E(\D)_{f,g}(\Gamma;H).
\end{align*}
The first step is cartesian factorization, the second is
\cref{thm:conservative}, the third is postcomposition with
$s_{f,H,g}^{-1}$, and the last takes the quotient class. The construction
is independent of the two local binding presentations by
\cref{eq:choicefreerel}.

Identities are preserved by the binding identities. For substitution of a
...
Finally, the test for an original unary cartesian cell is a bijection of
...
A \emph{boundary presentation} of a vertical arrow $f:X\to X'$ consists
...
\emph{equipment boundary transport} when every vertical arrow admits a
...
declare a multiwire merge to be a tree operation. A \emph{poly-virtual
...
\begin{equation}\label{eq:mates}
 \E(\D)_{f,g}(\Gamma;\Delta)
 \cong L(P)(\Gamma;f_*\Delta g^*)
 \cong L(P)(f^*\Gamma g_*;\Delta).
\end{equation}
...
\end{proposition}
\begin{proof}
The first bijection chooses the unique representative of the quotient class
in the selected summand of \cref{eq:choicefreecells}; equivalently it is
\cref{eq:globalcells} in that cloven presentation. For the second, cut the
...
\]
For its inverse, cut the $f^*$ output of $j_f$ into the leftmost input
...
\cref{eq:triangles}. Their naturality under tree substitution follows
...
these operations and equalities are retained by the reduct.
\end{proof}
...
\end{proposition}
\begin{proof}
For three arrows, independence of bracketing is the associativity of $k$
...
presentations. Expand an iterated bending into insertions of $j$, cuts
...
possible local changes of evaluation order. Cuts on disjoint wires commute
by interchange. Adjacent $j,e$ pairs cancel by the triangle identities
\cref{eq:triangles}. Rebracketings of companion or conjoint strings agree
by the just-proved independence of $K,L$. Replacing bending successively
along $f,u$ by bending along $uf$ is exactly
\cref{eq:unitcomp,eq:counitcomp}. Thus any two evaluations with the same
typed exterior reduce to the same expression. These calculations are
spelled out in \cref{app:bindings}. Finally, changing the local binding
...
Thus \cref{eq:globalembedding} retains every original square with
one atomic output, including its vertical maps. The mate bijections
\cref{eq:mates} give coherent transport even when the resulting boundary
has no atomic representative. Both facts will survive the reversal needed
for forward bicomodule maps.

\section{The completion theorem and maps of calculi}\label{sec:completion}

The global square calculus now supplies the two parts needed for the
general theorem: conservativity on original multicells and coherent
vertical transport. Taking its poly-virtual reduct retains the category
operations. We also determine which maps of the initial calculi extend
...
all their equations and compositions. Finally apply \cref{thm:reduct}.
\end{proof}
...
\end{theorem}
\begin{proof}
By \cref{thm:localUP}, extension is equivalent to invertibility of
every $F(s)$ in $L(P')$. By \cref{thm:detection}, this is precisely
membership in $S(P')$. For globalization, the image of an original unit comparison becomes
invertible. Hence the image binding cells define a companion and conjoint
presentation for the image vertical map in the target realization. Compare
...
image of \cref{eq:globalcells}. Their compatibility with $j,e,k,l$ proves
that \cref{eq:hcomp,eq:vcomp} are respected. The cocycle identity makes
the resulting quotient class independent of the local presentations, and
uniqueness proves compatibility with compositions of such functors.
\end{proof}
...
\end{proposition}
\begin{proof}
From a merge calculus, regard a boundary path as a horizontal arrow and
...
units, and interchange. Conversely, compose a rectangular expression
in a strict double category. The double-category laws identify the
generating equivalent schemes, so its value is well defined. These
...
The completion is therefore determined by its intrinsic comparison
class, and a map extends precisely when it preserves that class.
We apply it next to the bicomodule input, where reversal converts the
preserved single-output cells into forward balanced maps.

\section{Formal internal categories in an arbitrary monoidal base}\label{sec:formalinternal}

The conservative completion is now available for the virtual equipment
$\Ccal(\V)$ constructed in \cref{sec:bicomod}. Reversing its squares
restores the ordinary direction of bicomodule maps. The poly-virtual
reduct then gives the category operations of \cref{def:formalcat} in
every monoidal base.

\begin{definition}\label{def:Q}
The \emph{bicomodule poly-virtual equipment} is
...
A \emph{formal internal category in $\V$} is a formal category in
...
\end{theorem}
\begin{proof}
Apply \cref{prop:modinput,lem:bicomodunit,thm:completion}, then
...
and counit. Use the second mate description in \cref{eq:mates} to
...
required coherent equipment transport. The cell assertion is the
dual of \cref{thm:globalembedding}. The finite-expression assertion
is \cref{def:localization,def:global}; the latter's adjunction cells
were themselves constructed as finite expressions in
\cref{lem:binding}. The reduct changes none of these collections.
\end{proof}
...
Formal multiplication has been defined in every monoidal base. To compare
it with Aguiar's multiplication, we must show that an existing stable
cotensor represents the same input boundary and transports the same
equations. We first recognize its cone by contextual universality, then
use its inverse comparison to make the conversion by one cut.
...
\begin{tikzcd}[column sep=large]
 T \arrow[r,hook,"i_{M,N}"] & M\otimes N
 \arrow[r,shift left=.65ex,"\rho_M\otimes1"]
 \arrow[r,shift right=.65ex,"1\otimes\lambda_N"'] & M\otimes D\otimes N
\end{tikzcd}
\end{equation}
...
A stable cone \cref{eq:cotensorfork} has unique exterior coactions
...
\end{lemma}
\begin{proof}
The map $(\lambda_M\otimes1)i_{M,N}$ satisfies the balance equation
...
\[
 (1_C\otimes i_{M,N})\lambda_T=(\lambda_M\otimes1)i_{M,N}.
\]
Construct $\rho_T$ similarly. Postcomposing the two coassociativity
...
\]
through $1\otimes i_{M,N}\otimes1$, by stability. For every remaining
...
\end{proposition}
\begin{proof}
By \cref{lem:cotensorstructure}, the comparison in
$\Ccal(\V)$ belongs to $S(P)$. Its image is inverted in $L(P)$;
after reversal its inverse has the stated profile. Whiskered
precomposition with it is a bijection in the merge calculus. The map
...
Consequently it retains the bijection. The nullary assertion follows
identically from \cref{lem:bicomodunit}.
\end{proof}
...
\end{lemma}
\begin{proof}
For a triple, the object $(M\square_D N)\square_E K$, mapped into
...
\cref{lem:cotensorstructure}. Reversing the order of these two
factorizations gives $M\square_D(N\square_E K)$ and the same
cone functor. The unique mutually inverse factorizations define the
associator $a_{M,N,K}$, characterized by the commutative triangle
\[
\begin{tikzcd}[column sep=large]
 (M\square_D N)\square_E K
 \arrow[rr,"a_{M,N,K}"] \arrow[dr,"(i_{M,N}\otimes1)i_{M\square N,K}"'] &&
 M\square_D(N\square_E K)
 \arrow[dl,"(1\otimes i_{N,K})i_{M,N\square K}"]\\
 &M\otimes N\otimes K.&
\end{tikzcd}
\]
The tensor associator is understood on the lower vertex. Induction
establishes the same property for any finite path. Two routes around
...
For units, $\lambda_M:M\to C\otimes M$ is the equalizer of
...
the right coaction gives $M\square_D D\cong M$. Inserting a unit
...
by uniqueness. This also verifies the nullary cases of the induction.
\end{proof}
...
\begin{equation}\label{eq:mutransport}
 \mu=m\tau_{A,A},\qquad \eta=u\tau_0,
 \qquad m=\mu\iota_{A,A},\qquad u=\eta\tau_0^{-1},
\end{equation}
where the inverse formulas are evaluated in the merge presentation and
then identified with actual maps by unary full faithfulness.
In particular this recovers Aguiar internal categories in every regular
monoidal category.
\end{theorem}
\begin{proof}
By \cref{prop:tau} and unary full faithfulness from
\cref{thm:arbitraryV},
\begin{align*}
 \Q(\V)((A,A);A)&\cong
 \Q(\V)(A\square_C A;A)
...
\end{align*}
These are the formulas \cref{eq:mutransport}. Their forward
operations each use one cut. We verify all equations.

Write $T=A\square_C A$, $T_L=T\square_C A$, $T_R=A\square_C T$,
and let
\[
...
$I_Ra_{A,A,A}=I_L$. Functoriality of cotensor maps, defined by
...
Postcomposing with $\mu$ therefore transports the two associativity
trees to
...
Let $\ell_A:C\square_C A\to A$ and $r_A:A\square_C C\to A$
...
Aguiar's internal category is a monoid in this bicomodule monoidal
category \cite[Theorem 2.2.1 and Definition 2.3.1]{Aguiar1997};
regularity supplies every stable cone used above.
\end{proof}
...
\end{corollary}
\begin{proof}
Replace each boundary by its representative using the invertible cone
comparisons. This gives a unary square, and unary full faithfulness
identifies it with an actual equivariant map. Independence from
parenthesization and from the sequence of replacements follows from
\cref{lem:cotensorcoherence}. For adjacent squares, pasting the
...
maps have not been changed during any replacement.
\end{proof}

We have therefore recovered multiplication and its coherence from
the formal operations, and represented paths can be replaced throughout
the completed square calculus. To recover the full category of internal
categories, it remains to identify the equations for maps that change
the comonoid base.

\section{Changing-base functors and bimodule actions}\label{sec:changing}

The fixed-base comparison identifies category structures on each
endobicomodule. Global conservativity also retains equivariant maps over
comonoid maps. We use the universal cotensor cones to show that their
formal homomorphism equations are exactly the internal-functor equations.
The same calculation then applies to bimodule actions and their maps.
...
\begin{equation}\label{eq:changefactor}
 i_{M',N'}(F\square_g G)=(F\otimes G)i_{M,N}.
\end{equation}
These maps respect identities and composition of all three base maps.
\end{lemma}
\begin{proof}
Using equivariance, the two balance maps of the right side of
\cref{eq:changefactor} are
\[
...
Postcompose its proposed exterior equivariance equations with
$1\otimes i_{M',N'}$ or $i_{M',N'}\otimes1$. The results follow
from exterior equivariance of $F,G$; stability makes these testing maps
monic. Identity maps satisfy the characterization, so their factor is
the identity. For successive pairs $(F,G)$ and $(F',G')$, either
...
\end{theorem}
\begin{proof}
Unary global full faithfulness from \cref{thm:arbitraryV} identifies
the formal carrier square with exactly the equivariant map $F$.
For multiplication, replace the upper boundary $(A,A)$ by
...
\end{tikzcd}
\]
For units, the primitive map $f:C\to D$ intertwines the regular
...
Full faithfulness reflects both equations. The identity and composite
...
in the cotensor formulation \cite[Section 4.1]{Aguiar1997}.
\end{proof}
...
such that
\begin{align}
 p(\mu_A,1)&=p(1,p), & p(\eta_A,1)&=1_M,\label{eq:leftaction}\\
 q(q,1)&=q(1,\mu_B), & q(1,\eta_B)&=1_M,\label{eq:rightaction}\\
 q(p,1)&=p(1,q).&&\label{eq:commutingactions}
\end{align}
...
For the second assertion, apply \cref{prop:tau} to $A,M$ and $M,B$
and then use unary full faithfulness. The left associativity equation,
...
The commuting equation, tested on $(A\square_C M)\square_D B$,
becomes
\[
 q_0(p_0\square_D1_B)
   =p_0(1_A\square_Cq_0)a_{A,M,B}.
\]
The unit equations become the left and right cotensor unitors, by the
nullary calculation in \cref{thm:aguiar}. Finally
\cref{lem:changebase} transports \cref{eq:modulemap} to the two
displayed equivariant-map equations. Every step is reversible.
\end{proof}
...
A cell of \cref{eq:carriercells} has one output \emph{carrier}, even
...
\end{lemma}
\begin{proof}
The strict double-category laws of \cref{thm:globalcoherence}, retained
...
Use the reversed companion and conjoint data from
...
$\chi_{f,K,g}:f_*Kg^*\cell K$. To verify its full cartesian property,
...
carriers. This is cartesianness in $\Kcal(\V)$.
\end{proof}
...
distributor $M:\mathsf A\harr\mathsf B$ consists of a carrier
...
such that
\begin{align}
 p_M(\mu_A,1)&=p_M(1,p_M),&
 p_M(\eta_A,1)&=1_{H_M},\label{eq:distleft}\\
 q_M(q_M,1)&=q_M(1,\mu_B),&
 q_M(1,\eta_B)&=1_{H_M},\label{eq:distright}\\
 q_M(p_M,1)&=p_M(1,q_M).&&\label{eq:distcommute}
\end{align}
These are substitution equations in $\Kcal(\V)$, hence specified merge
equations in $\mathbb B(\V)$. We call a distributor
\emph{bicomodule-carried} when $H_M$ consists of one actual bicomodule.
\end{definition}

On a bicomodule-carried distributor every complete expression in
\cref{eq:distleft,eq:distright,eq:distcommute} is an original tree
evaluation. Thus these distributors are exactly
\cref{def:bimodule}, with exactly its equations. For general path
carriers the merge realization supplies the required evaluations.
...
\]
For $n>0$, a distributor multicell over $(\mathsf F,\mathsf G)$ is
a cell
\[
 \alpha\in\Kcal(\V)_{f,g}(H_{M_1},\ldots,H_{M_n};H_N)
\]
satisfying exterior equivariance
\begin{align}
 \alpha(p_{M_1},1,\ldots,1)&=p_N(F,\alpha),
 \label{eq:distexteriorleft}\\
 \alpha(1,\ldots,1,q_{M_n})&=q_N(\alpha,G),
 \label{eq:distexteriorright}
\end{align}
and interior balance
\begin{equation}\label{eq:distbalance}
 \alpha(\ldots,q_{M_i},1_{H_{M_{i+1}}},\ldots)
 =\alpha(\ldots,1_{H_{M_i}},p_{M_{i+1}},\ldots)
 \quad(1\leq i<n).
\end{equation}
Both sides of \cref{eq:distbalance} have the carrier $A_i$ inserted
between $H_{M_i}$ and $H_{M_{i+1}}$.

For $n=0$, the two functors have common domain $\mathsf A$, the
underlying cell is $\alpha:\emp X\cell H_N$ over $(f,g)$, and
the condition is
\begin{equation}\label{eq:distnullary}
 p_N(F,\alpha)=q_N(\alpha,G).
\end{equation}
The common source in this equation is the carrier $A$ of $\mathsf A$.
\end{definition}
...
\end{proposition}
\begin{proof}
Define substitution by that of $\Kcal(\V)$. Consider an inserted
...
two results. At the interface between two nonempty blocks, their exterior
equations move the action to the corresponding inputs of the outer cell;
the outer balance equation then identifies the results. At an empty block,
...
At the two exterior boundaries, the same calculation uses the exterior
...
Unary identity cells satisfy exterior equivariance, and have no interior
balance equations. Associativity and identities now follow from the
corresponding identities in $\Kcal(\V)$. The definition of a monoid in
...
\cite[Definition 2.8]{CruttwellShulman2010}, used here.
\end{proof}
...
\end{lemma}
\begin{proof}
The monad laws make the displayed data a distributor, and the two unit
laws give \cref{eq:distnullary} for $\eta_A$. We prove the contextual
...
agree when both are available. Associativity of the adjacent action
...
If both contexts are empty, write the boundary functors as
...
Substitution of $\eta_A$ into the inserted factor recovers $\alpha$
by the action unit laws. Conversely, for a balanced cell $\beta$ with
...
\begin{align*}
 ...
 =\chi p_H(1,p_H).
\end{align*}
For the left unit, the functor unit equation and the action unit equation
give
\[
 \chi p_H(\eta_A,1)
 =p_N(F\eta_A,\chi)
...
\]
Right associativity and its unit equation use $G$ in the same way.
For commuting actions,
\begin{align*}
 \chi q_H(p_H,1)
...
To verify cartesianness in $\Ddist(\V)$, take a distributor multicell
...
For further functors $\mathsf R=(r,R)$ and $\mathsf S=(s,S)$,
...
comparison is an invertible distributor map. The identity comparisons and
every association comparison respect the same cartesian cells; uniqueness
proves their coherence.
\end{proof}
...
\end{corollary}
\begin{proof}
Combine \cref{prop:distvirtual,lem:distunit,thm:distrestriction}.
...
The distributor action equations provide a single-output calculus. We now
apply the conservative completion to this new virtual equipment. This
supplies the many-sided distributor cells and their boundary transport.
The comparison class at this stage is computed from balanced distributor
multicells, so it records representation of distributor paths.
...
\end{theorem}
\begin{proof}
Apply \cref{thm:completion,thm:globalembedding} to the virtual equipment
...
poly-virtual equipment structure. The finite-expression assertion is the
...
existing finite-term calculus and the displayed equations.
\end{proof}
...
\end{corollary}
\begin{proof}
The indicated restriction on horizontal arrows is closed under virtual
substitution, which does not form a new carrier. It contains the regular
distributor at every object. Its multicells are the same as those of
$\Ddist(\V)$ on these profiles, so \cref{eq:distconservative}
gives the assertion. For unary cells, \cref{thm:arbitraryV} identifies
the underlying square with an actual equivariant bicomodule map, and
\cref{eq:distexteriorleft,eq:distexteriorright} become exactly
\cref{eq:modulemap}.
\end{proof}
...
\end{proposition}
\begin{proof}
The two restrictions in \cref{eq:distcompanions} exist by
\cref{thm:distrestriction}; \cref{eq:distrestrictioncarrier} computes
their carriers. Use these restriction cells and the unit comparison of
...
for composite functors. \Cref{prop:mates} gives
...
The unit and counit composition equations prove compatibility with
vertical composition, and tree substitution compatibility follows by
substituting the same binding cuts into each tree. Identity companions
...
Two distinct replacements can occur in the distributor calculus. A stable
cotensor can replace the bicomodule path underlying one distributor.
A relative tensor product can replace a path of distributors by one
distributor. The first replacement uses the original comparison class;
the second uses the comparison class of $\Ddist(\V)$.
...
\end{lemma}
\begin{proof}
The equations for $h$ to preserve the actions force
\cref{eq:disttransport}. Substitution cancels each adjacent
...
For a multicell, postcompose with the comparison on its output and
...
Inverse transport uses the inverse comparisons, and substitution
compatibility follows from cancellation at every internal carrier.
\end{proof}
...
\end{corollary}
\begin{proof}
Use the inverse cone comparison as $h$ in
...
and equivariant maps. For multicells, transport every carrier first.
The bijection of \cref{lem:distcarriertransport} respects their balance
and exterior equations. For a stably represented source path, unary full
...
balancing of that map. Cone uniqueness gives independence of the chosen
parentheses in each represented path.
\end{proof}
...
\end{proposition}
\begin{proof}
Apply \cref{thm:detection,thm:globalembedding} to $\Ddist(\V)$.
In the merge realization, contextual precomposition with an invertible
cell is a bijection. The forward map in \cref{eq:distconversion} is
...
The converse is the detection theorem for this second comparison class.
\end{proof}
...
\end{theorem}
\begin{proof}
Let $A,C$ be the carriers of the two exterior formal categories. The
...
Every contextual instance of $c$ is an epimorphism, since it is a
...
For contextual universality, let
...
unique underlying factor $\bar\alpha:(L,P,R)\cell Z$. To verify
...
That contextual instance is epic, so the equation holds. Conversely,
...
Globular detection in the virtual equipment $\Ddist(\V)$ gives all
...
of the particular fork \cref{eq:distcoequalizer}.
\end{proof}

When these representations exist for successive distributor pairs, their
iterated composites have the usual coherent associators and unitors.
...
The examples test two features of the definition. The unit comonoid and
Cartesian bases recover familiar category structures, while the identity
category exists over every comonoid without a global exactness condition.
The example in $\Ab$ then separates ambient regularity from the stability
of the particular cotensors used for composition. On its finite
group-like sector we also construct the relative tensor products of
distributors and recover the corresponding enriched equipment.
...
\end{proposition}
\begin{proof}
The counit law forces either $I$-coaction to be the inverse unitor.
The two maps defining balance over $I$ are therefore equal after
...
\Cref{thm:aguiar,thm:globalaguiar} identify the resulting structures
and fixed-base maps with ordinary monoids and their homomorphisms.
\end{proof}
...
\end{proposition}
\begin{proof}
By \cref{lem:bicomodunit}, the comparison
...
\end{proposition}
\begin{proof}
Every object has its unique diagonal comonoid. Counitality forces a
...
internal graph with its two endpoint maps. The balance equalizer of
...
sides, so the cone is stable. The maps and equations of
...
\end{proposition}
\begin{proof}
The coreflexive pair $r,s:\mathbb Z\rightrightarrows\mathbb Z\oplus
\mathbb Z$ given by
\[
...
For the specified comonoid write
\[
...
Decompose $M\otimes N$ into the summands
...
This argument remains valid after arbitrary tensoring on either side:
...
For $A:C\harr C$, a multiplication consists of maps
...
\cref{thm:aguiar} to these stable cones.
\end{proof}

\begin{remark}
The preceding identification includes the usual variation of the finite
...
gives the usual enriched functors.
\end{remark}
...
distributor path is represented by its usual relative tensor product.
...
\end{proposition}
\begin{proof}
Let $\mathbb B_{\mathrm{fin}}$ be the specified part of
...
\Cref{prop:Ab} and its proof, with rectangular indexing sets, identify
...
representatives. \Cref{lem:distcarriertransport} replaces every
...
distributor. The exterior equations on maps give the usual equivariant
...
For distributors $M:\mathsf A\harr\mathsf B$ and
...
belong to the $j'$ and $j$ summands, respectively. This is the
componentwise coequalizer of the two middle actions. Tensor product of
abelian groups preserves coequalizers in either variable, and finite
...
\Cref{thm:distcoequalizer} equips
\cref{eq:Abdistributorproduct} with its exterior actions and proves
...
representing comparisons. In the second completion, replace both
...
assertion, including arbitrary exterior enriched functors.
\end{proof}
...
\end{lemma}
\begin{proof}
Substitution composes the arrows at cut vertices as well as the root
...
Nullary blocks use the degenerate virtual pastings, and the structural
empty square on $f$ is its horizontal identity. For one output there
is one block and its two cut arrows are the specified $f,g$, so the
displayed equality follows. All monoid and homomorphism operations lie
in this fragment, which proves the last statement.
\end{proof}
...
\end{theorem}
\begin{proof}
A horizontal Kleisli arrow from $X$ to $Y$ is the original arrow
...
unit are the monoid cells in $\mathbb K$. Apply \cref{lem:raw} and
\cref{thm:reduct}. If $\mathbb K$ is an equipment, apply
\cref{thm:completion} instead; its inverted comparisons are precisely
the intrinsic ones by \cref{thm:detection}.
\end{proof}
...
\end{proposition}
\begin{proof}
Normalization is the cartesianness of this unit square
\cite[Definition 8.2]{CruttwellShulman2010}. Its top is atomic and its
bottom is atomic in the original equipment $\mathbb X$.
\Cref{thm:globalembedding} preserves and reflects all the single-output
factorization tests defining cartesianness. The object correspondence in
\cref{thm:kleisli} retains the original unit square as data and
therefore retains this property.
\end{proof}
...
\end{proposition}
\begin{proof}
Garner constructs the two-sided Kleisli bicategory using a
...
For any bicategory, fix a bracketing of each finite path of $1$-arrows
...
evaluation order \cite[Section 1.5]{Leinster2004}. A monad on
one original $1$-arrow is exactly \cref{eq:shape,eq:monadlaws} in
this path presentation. Apply \cref{thm:reduct}. The symmetric
permutations here are part of Garner's Kleisli data; this argument does
not insert exchanges into our planar wire calculus.
\end{proof}

These comparisons return to the enriched and generalized-multicategory
precedent that motivated the construction. A single tree definition of
formal category accommodates those virtual examples and the forward
bicomodule realization; the latter uses the conservative completion to
recognize represented cotensors on its input boundary.

\section{Universal characterization and comparison with other envelopes}\label{sec:comparison}

The construction has been used to preserve virtual monoids, to define
formal internal categories, and to construct a poly-virtual equipment of
their distributors. Its universal property specifies the common
mechanism: invert the intrinsic contextual comparisons of the chosen
input after forming forests. In the two applications the inputs, and
therefore the comparison classes, are different. We record what this
characterizes and what is needed for a comparison with an independently
defined envelope.
...
\end{theorem}
\begin{proof}
The presentation and its uniqueness are \cref{thm:localUP}.
The exact recognition of reversible atomic comparisons is
\cref{thm:detection}. The two conservativity assertions are
\cref{thm:conservative,thm:globalembedding}. For the reduct use
\cref{thm:reduct,prop:mates,thm:bimodules}.
For the distributor instance apply \cref{thm:distcompletion}.
\end{proof}

The path presentation in \cref{prop:pathpresentation} is the merge
form of the two-sided calculus. The distinction between virtual and
implicit boundary operations is also described in
\cite[Remark 1.1]{FairbanksShulman2025}. The related frameworks
enter the argument in the following roles.
\begin{center}
\small
...
Framework & Role\\
\midrule
Virtual double categories & Single-output operations and generalized
multicategory presentations.\\
Poly-bicategorical calculi & Two-sided boundaries with planar tree
composition.\\
Merge and implicit calculi & Evaluation along entire boundaries and
reversal of represented comparisons.\\
Module or presheaf envelopes & Other realizations defined by their own
representability or cocompletion properties.\\
The module construction on path carriers & Formal distributors and their
cartesian restrictions, followed by a second conservative completion.\\
Two-sided Kleisli bicategories & Arity-sensitive examples, including
Garner's symmetric polycategories.\\
\bottomrule
\end{tabular}
\end{center}
...
\end{proposition}
\begin{proof}
The extension statement is \cref{thm:localUP}. A fully faithful
...
functor is a quasi-inverse by those same isomorphisms. Conversely an
equivalence is fully faithful and essentially surjective. Applied
to $\bar J$ this gives the stated test.
\end{proof}
...
The guiding problem was to retain non-Cartesian category operations when
ambient regularity is unavailable. The construction separates two roles
of cotensors: a finite path specifies the boundary of an operation, while
a stable cotensor represents that boundary by one bicomodule. Formal
internal categories use the former in every monoidal category and recover
the latter whenever it is available.

Conservativity is what makes this separation compatible with the original
data. Forward equivariant bicomodule maps, their changes of base, and all
primitive balanced cells remain unchanged. The reverse comparison for a
represented cotensor then turns a formal multiplication into an actual
cotensor multiplication, and uniqueness of its cone transports
associativity and units. Thus the recovery theorem identifies the category
structures, their equations, and their changing-base functors.

Formal categories themselves are the objects of the second equipment
$\FDist(\V)$. Its vertical arrows are their formal functors, and
its horizontal arrows have distributor actions on finite bicomodule-path
carriers. Cartesian factorization constructs the restrictions and proves
their action laws. The regular distributors give represented units, and
the second conservative completion supplies the many-sided cells and
coherent companion/conjoint transport. Actual bicomodule distributors and
their equivariant maps remain fully faithful in this construction.

Representation at the distributor level balances the actions of the
intervening formal category. Contextually preserved action coequalizers
construct these relative tensor products and give their coherence by
uniqueness. The finite group-like $\Ab$ sector verifies both stages:
stable cotensors recover enriched category operations, and action
coequalizers recover enriched distributor composition, in an ambient
monoidal category that is not regular.

The two completions therefore extend the virtual treatment of composition
through two levels of category theory. They retain finite paths as
legitimate boundaries, make existing intrinsic representations reversible,
and preserve the original single-output cells at each completion stage.
After the first reversal, this is the forward balanced single-input
bicomodule fragment; at the second stage it is the balanced distributor
fragment. All constructions use finite expressions, with size understood
as in the following appendices.
...
\end{proposition}
\begin{proof}
For small data, finite paths and finite subdivisions form sets.
Forests are finite products of original cell sets, indexed by a set
of subdivisions, hence form a set. The collection $S(P)$ is a
...
relation on it, and its quotient is a set. For a fixed vertical arrow $f$,
...
For the distributor construction on small data, paths and the cells of
...
For large data the same description uses finite terms labelled in
...
universe. The conservativity theorem does give exactly the original
...
smallness of its general many-sided profiles in a smaller universe.
\end{proof}
...
Richard Garner,
\emph{Polycategories via pseudo-distributive laws},
...
Amar Hadzihasanovic,
\emph{Weak units, universal cells, and coherence via universality for
bicategories},
...
Michael Shulman,
\emph{Framed bicategories and monoidal fibrations},
